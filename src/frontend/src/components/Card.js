...
export default function GithubCard(props) {
  ...
  var headerData = {method: "GET", headers: {authorization: "token " + localStorage.getItem('apikey')}}

	function getData() {
		fetch(props.apiURL, headerData)
	...
	function getRateLimit() {
		fetch('https://api.github.com/rate_limit', headerData)
			.then(response => response.json())
			.then(data => setRateLimit(data.rate.remaining))
	}

	function getColors() {
		fetch("https://raw.githubusercontent.com/owenmoogk/github-card/7848459fc0f4c252850d576b1131e7dfd09c22a7/public/colors.json")
			.then(response => response.json())
			.then(data => setColors(data))
	}

	function getEmojis() {
		fetch("https://raw.githubusercontent.com/owenmoogk/github-card/main/public/emojis.json")
			.then(response => response.json())
